...
def train(base_loader, val_loader, model, start_epoch, stop_epoch, params, base_loader_u=None):    
    ...
    for epoch in range(start_epoch,stop_epoch):
        model.train()
        model.train_loop(epoch, base_loader,  optimizer, writer, base_loader_u=base_loader_u) #model are called by reference, no need to return 
        model.eval()
...
if __name__=='__main__':
    ...
    if os.path.exists('/local/jcsu_datasets/inat_2018'):
        import shutil
        print('start removing')
        shutil.rmtree('/local/jcsu_datasets/inat_2018')
    ...
    if params.dataset_unlabel is not None:
        base_file_unlabel = os.path.join('filelists', params.dataset_unlabel, params.base_unlabel+'.json')
    ...
    if params.method in ['baseline', 'baseline++'] :
        base_datamgr_l    = SimpleDataManager(image_size, batch_size = params.bs, jigsaw=False, rotation=False, isAircraft=isAircraft, grey=params.grey)
        base_loader_l     = base_datamgr_l.get_data_loader( base_file , aug = params.train_aug )

        base_datamgr_u    = SimpleDataManager(image_size, batch_size = params.bs, jigsaw=params.jigsaw, rotation=params.rotation, isAircraft=isAircraft, grey=params.grey)
        if params.dataset_unlabel is not None:
            base_loader_u     = base_datamgr_u.get_data_loader( base_file_unlabel , aug = params.train_aug )
        else:
            base_loader_u     = base_datamgr_u.get_data_loader( base_file , aug = params.train_aug )
        ...
        base_datamgr_l            = SetDataManager(image_size, n_query = n_query,  **train_few_shot_params, isAircraft=isAircraft, grey=params.grey)
        base_loader_l             = base_datamgr_l.get_data_loader( base_file , aug = params.train_aug )
    ...
    train(base_loader_l, val_loader,  model, start_epoch, stop_epoch, params, base_loader_u=base_loader_u)
